...
namespace AWSLambdaQuoteRequest;
...
public class Function
{
    ...
    private async Task ProcessMessageAsync(SQSEvent.SQSMessage message, ILambdaContext context)
    {
        ...
        emailMessage.To.Add(new MailboxAddress(request.HandicraftAuthorName, request.HandicraftAuthorEmail));
        emailMessage.Subject = $"New Quote Request on Your Handicraft: {request.HandicraftName}";

        // Create HTML body
        var htmlBody = new TextPart("html")
        {
            Text = $@"<h1>New Quote Request</h1>
              ...
              <p>You can reply to this email to get in touch with the person who requested the quote.</p>"
        };
        ...
        // TODO: change to the RDS connection string
        using (SqlConnection connection = new SqlConnection(dbConnectionString))
        {
            connection.Open();

            // Update the QuoteRequest to IsProcessed = true
            using (SqlCommand command = new SqlCommand("UPDATE QuoteRequests SET IsProcessed = 1 WHERE Id = @Id", connection))
            {
